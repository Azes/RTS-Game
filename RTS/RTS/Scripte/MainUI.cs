...
public class MainUI : MonoBehaviour
{
    ...
    //tmp für die anzahl der einheiten 
    ...
    void Update()
    {
        ...
        if (humans.Count > 0)
        {
            ...
            //wenn der buffer sich verändert wwchseln wir zwischen den infos objecten
            if (infoToggle != s)
            {
                infoToggle = s;
    ...
    void isListValueChange(bool it)
    {
        if (it)
        {
            //wird überprüfen ob sich die anzahl der child objecte also auch die anzahl
            //der einheiten verändert hat in dem fall fügen wir die einheit in der group ui 
            //entweder hinzu oder löschen sie wenn nicht mehr ausgewählt

            //buff für zulöschende grou child objecte
            List<GameObject> buff = new List<GameObject>();

            //überprüft ob die ui group child objects mit der in humans aufgelisten
            //einheiten übereinstimmen wenn nicht kommt sie in den lösch buffer
            ...
            //erst die buttens dann sortedList hinzufügen
            sortedList();
        }
